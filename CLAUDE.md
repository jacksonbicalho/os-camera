...
`CamerasSettingsPage` (`/settings/cameras`) — lista câmeras com thumbnail do snapshot (`GET /api/cameras/{id}/snapshot`). Exibe badge "motion" para câmeras com detecção ativa e badge "rec off" para câmeras com `recording_enabled=false`. No modo admin, suporta drag-and-drop para reordenar: handle de arrastar (⠿) à esquerda de cada linha, destaque azul no alvo, atualização optimista do estado local e chamada a `PUT /api/settings/cameras/reorder` ao soltar.

`StorageSettingsPage` (`/settings/storage`) — duas seções: **Retenção** e **Drives**. Retenção: uma linha por categoria (com movimento / sem movimento) com `DurationInput` (valor + unidade), seletor de ação (`apagar` / `enviar para drive`) e seletor de drive (visível apenas quando ação = `send_to_drive`); configurações de intervalo, tamanho máximo e alerta de uso na mesma tela; salvar chama `PUT /api/settings/storage` que persiste overrides no banco e dispara `ForceClean` no servidor imediatamente. Drives: lista com nome, endpoint, bucket e prefixo (credenciais exibidas como `****`); formulário modal para criar/editar; exclusão com `ConfirmDialog`. `DurationInput` é declarado fora do componente (requisito do lint `react-hooks/static-components`). Overrides locais usam o padrão `overrides` state para evitar `setState` em `useEffect`.
...
| `internal/storage` | `Cleaner` com retenção diferenciada por categoria e ações configuráveis. Quando o banco está disponível: `syncRecordings()` varre o filesystem, sincroniza MP4s para a tabela `recordings` (`INSERT OR IGNORE`) e atualiza `has_motion` via join com `motion_events` respeitando janela de lead/trail configurada em `camera_motion`; `cleanFromDB()` consulta `recordings` para decisões de exclusão — ação `delete` remove o arquivo localmente, ação `send_to_drive` faz upload via interface `Drive` e depois purga arquivo + registros do banco (falha no upload mantém o arquivo para retry na próxima rodada). Sem banco: consulta `motion.ndjson` diretamente. O `Run()` usa um ticker fixo de 1 minuto; o intervalo efetivo é lido do banco a cada tick (`storage.interval_minutes`), então mudanças entram em vigor em até 1 minuto sem restart. `ForceClean()` dispara um ciclo imediato via canal bufferizado (capacidade 1). Interface `Drive` (`drive.go`): `Upload(ctx, key string, r io.Reader, size int64) error`. Implementação `S3Drive` (`s3drive.go`): AWS Signature V4 sem SDK externo; suporta path-style (MinIO) e virtual-hosted; chave S3 preserva a estrutura de pastas `{prefix}/{camera_id}/{YYYY}/{MM}/{DD}/{filename}.mp4`. |
| `internal/db` | Acesso ao SQLite (`modernc.org/sqlite`). Executa migrations em `internal/db/migrations/` na inicialização. Tabelas: `cameras`, `camera_recording_config`, `camera_motion_config`, `camera_motion_zones`, `users` (com `must_change_password`), `settings`, `recordings`, `motion_events`, `drives`, `retention_config`. `drives.go`: CRUD de drives S3 e leitura/escrita de `retention_config` (categoria → ação + drive_id). `recordings.go`: `HasMotionByPaths(db, paths)` — consulta bulk de `has_motion` por caminho de arquivo. |
| `internal/ffprobe` | Executa e parseia saída JSON do ffprobe para detectar codec, áudio e dimensões do stream. |
| `internal/server` | HTTP server com JWT HS256 (segredo gerado a cada boot, expira em 24h). O claim `must_change_password=true` bloqueia todos os endpoints exceto `POST /api/auth/change-password`. Serve API REST, arquivos de gravação (incluindo snapshots `_motion.jpg`), segmentos HLS e a SPA React. Endpoints SSE de movimento: `/api/cameras/{id}/motion/live` e `/api/cameras/{id}/motion/scores`. `GET /api/stats` usa `SUM(size_bytes)` da tabela `recordings` quando banco disponível. `PUT /api/settings/cameras/reorder` (admin) — reordena câmeras em lote via `{"ids":[...]}`. A rota estática `/reorder` é registrada antes de `PUT /api/settings/cameras/{id}` para ter precedência no mux do Go 1.22+. `display_order` não é aceito nos endpoints de create/update; o update preserva a ordem existente da câmera. `GET /api/cameras/{id}/recordings` retorna `has_motion` lido do banco (`HasMotionByPaths`) — gravações de contexto (lead/trail) também exibem o indicador na UI. Endpoints de drives e armazenamento (admin): `GET/POST /api/drives`, `PUT/DELETE /api/drives/{id}`, `GET /api/retention`, `PUT /api/retention/{category}`, `GET/PUT /api/settings/storage`. `PUT /api/settings/storage` salva overrides no banco (`system_config`) e chama `cleaner.ForceClean()` automaticamente. Credenciais S3 nunca aparecem em respostas de API. |
