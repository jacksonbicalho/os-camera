...
O padrão de **override files** mantém `docker-compose.yml` universal (funciona em RPi, AMD, CPU-only) e `docker-compose.nvidia.yml` adiciona apenas o device reservation NVIDIA. Nunca colocar configuração de GPU no `docker-compose.yml` base. Para **webcam local** (v4l2), o compose base monta `/dev` + `device_cgroup_rules: ["c 81:* rmw"]` nos serviços `camera`/`camera-dev` — funciona no `up` normal e é inócuo sem webcam (montar `/dev` não falha; o cgroup só libera vídeo). **Nunca** usar `devices: /dev/video*` no base (quebraria hosts sem o device). Ver `docs/webcam.md`.
...
| `internal/webcam` | Webcam local (v4l2) como fonte RTSP, **self-contained** (só ffmpeg; sem Docker/binário externo). `List`/`Detected` enumeram `/sys/class/video4linux/*/name` (dedupe por nome → menor índice). `Manager` (ligado ao `ctx` em `main.go`) hospeda um **servidor RTSP embutido** (`gortsplib`, loopback `127.0.0.1:8554`) e supervisiona **um** ffmpeg por webcam que lê `/dev/videoN` e publica `rtsp://127.0.0.1:8554/webcamN` (H.264); os 3 processos do pipeline consomem como câmera RTSP comum. Só sobe algo se houver webcam. O Discover (`internal/discovery`) inclui as webcams como `Result{Kind:"webcam"}` e o frontend (`DiscoverPage`) mostra o método "Webcam" + "Adicionar" direto (sem credenciais). Ver `docs/webcam.md`. |
