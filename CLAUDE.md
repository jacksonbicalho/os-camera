...
`CameraPage` — além das gravações e eventos, exibe abaixo do player uma barra de controles com: botão mudo (sincronizado com os controles nativos via `onVolumeChange`), seletor de velocidade 1×–32× (detecta limite do browser progressivamente e exibe aviso), toggle "Reprodução contínua" (avança automaticamente para a próxima gravação ou próximo evento ao terminar o clip atual). Usa refs (`continuousPlayRef`, `activeEventIdxRef`, `visibleEventsRef`, `recordingsRef`) para evitar closures stale nos handlers de vídeo.
